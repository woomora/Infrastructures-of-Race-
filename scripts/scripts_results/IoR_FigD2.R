...
tranvias <- 
  st_read("data/source/historical_maps/tranvias/tranvias.shp") |> 
  st_transform(default_crs)

sf_use_s2(F)
convex_hull_trams <- st_convex_hull(st_union(tranvias)) 
convex_hull_trams <- st_bbox(convex_hull_trams)
...
p <- 
  ...
  geom_sf(
    data = tranvias, color = "orange", size = 2.5, lwd = .5
  ) +
...
p +
  ...
  coord_sf(
    xlim = c(convex_hull_trams[1], convex_hull_trams[3]),
    ylim = c(convex_hull_trams[2], convex_hull_trams[4]),
  ) +
...
ggsave(
  str_c(
    output_dir, "/historical_mechanisms_tramways.png"
  ),
  width = 6.66, height = 8
)
